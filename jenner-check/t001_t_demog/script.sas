/* t_demog.sas — demographics summary table (age / gender / race by treatment).
   Data ingest adapted for a self-contained run: the original PROC IMPORT read
   demog.xls from an absolute path; here WORK.DEMOG is built inline with the same
   columns (DAY MONTH YEAR DIAGDT GENDER RACE TRT), so every DATA step, PROC MEANS,
   PROC FREQ, PROC SQL, PROC TRANSPOSE and PROC REPORT below runs exactly as written. */

data WORK.DEMOG;
  length subjini $3;
  input subjini $ diagdt day month year gender race trt;
  informat diagdt date9.;
  format diagdt date9.;
  datalines;
A-D 15JUN2021 12  3 1975 1 1 0
S-S 15JUN2021  5 11 1968 2 2 0
MMP 15JUN2021 23  7 1982 1 4 0
KMC 15JUN2021 30  1 1959 2 1 0
VCB 15JUN2021 17  9 1971 1 3 0
SSN 15JUN2021  2  4 1990 2 5 0
SVI 15JUN2021 28 12 1963 1 2 0
KRN 15JUN2021  9  6 1978 2 4 0
MSK 15JUN2021 14  2 1985 1 1 1
TVD 15JUN2021 21  8 1972 2 3 1
BPS 15JUN2021  6  5 1966 1 2 1
SKB 15JUN2021 19 10 1988 2 1 1
JBS 15JUN2021 11  1 1970 1 4 1
RVK 15JUN2021 25  7 1980 2 5 1
SBG 15JUN2021  3 12 1962 1 1 1
LRD 15JUN2021 16  4 1977 2 2 1
;
run;
...
/* Added for this captured run only: print the transposed summary table
   (t_allstats) so a plain `curl` shows the computed Placebo/Active/All figures.
   The analytical pipeline above is your code, run as written. */
proc print data=t_allstats noobs label;
  label stat='Statistic' _0="Placebo (N=&placebo)"
        _1="Active treatment (N=&active)" _2="All patients (N=&total)";
  var stat _0 _1 _2;
run;
